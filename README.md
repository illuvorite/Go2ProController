...
1. **连接**：启动会自动连接命令行给的 IP；否则点「扫描局域网」或输入 IP 后点「添加」
2. **勾选**：在「设备」页勾选要控制的机器狗（1 台单控 / 多台群控）
3. **遥控**：拖左杆走位、右杆转向，松手即停；`空格键` 或红色「急停」按钮立即停车
4. **动作**：「动作库」页点各类动作；开关型指令点一次开、再点一次关
5. **急停后**：把两个摇杆回中 → 点「解除急停」才能继续遥控
